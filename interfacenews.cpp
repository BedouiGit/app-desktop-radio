...
void interfacenews::readSerial()
{
    ...
    if(buffer_split.length() < 3){
        ...
        interfacenews::updateTemperature(parsed_data);
    }

}

/*void interfacenews::updateTemperature( QString sensor_reading)
{
...
void interfacenews::on_video_on_clicked()  // PLAY VIDEO
{
...
void interfacenews::on_video_off_clicked()  // STOP VIDEO
{
    player->stop();
    vw->close();
}
...
void interfacenews::on_temperature_clicked()
{
    ui->templcdNumber->display(serialData);
}
void interfacenews::update_label()
{
    if (serialData.length()>=6)
        serialData="";
    serialData+=AA.read_from_arduino();
qDebug() << serialData;
